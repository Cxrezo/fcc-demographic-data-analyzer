...
def calculate_demographic_data(print_data=True):
    ...
    num_min_workers_above_50k = round(len(df[(df["hours-per-week"] == min_work_hours) & (df["salary"] == ">50K")]),1)
    ...
    highest_earning_country_percentage = round((df[df["salary"] == ">50K"]["native-country"].value_counts() / df["native-country"].value_counts()*100).sort_values(ascending=False).max(),1)
